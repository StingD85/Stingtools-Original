...
namespace StingBIM.Core.Logging
{
    ...
    public sealed class StingBIMLogger
    {
        ...
        // Static default logger for static method calls
        private static readonly Lazy<StingBIMLogger> _defaultLogger =
            new Lazy<StingBIMLogger>(() => new StingBIMLogger("StingBIM"));
        private static StingBIMLogger Default => _defaultLogger.Value;

        #endregion

        #region Static Logging Methods (Convenience)

        // These static methods allow calling StingBIMLogger.LogDebug(...) etc. directly
        // without creating an instance first

        /// <summary>Static debug logging</summary>
        public static void LogDebug(string message) => Default.Debug(message);

        /// <summary>Static info logging</summary>
        public static void LogInfo(string message) => Default.Info(message);

        /// <summary>Static warning logging</summary>
        public static void LogWarning(string message) => Default.Warn(message);

        /// <summary>Static error logging</summary>
        public static void LogError(string message) => Default.Error(message);

        /// <summary>Static error logging with exception (message first)</summary>
        public static void LogError(string message, Exception ex) => Default.Error(ex, message);

        /// <summary>Static error logging with exception</summary>
        public static void LogError(Exception ex, string message = null) => Default.Error(ex, message);
            ...
            [CallerLineNumber] int lineNumber = 0)
        {
            if (_logger.IsWarnEnabled)
            {
                var logEvent = CreateLogEvent(LogLevel.Warn, message ?? exception.Message, memberName, filePath, lineNumber);
        ...
        public void Warning(
            ...
            [CallerLineNumber] int lineNumber = 0)
        {
            Warn(message, memberName, filePath, lineNumber);
        }

        /// <summary>
        /// Alias for Warn - logs a warning message with exception
        /// </summary>
        public void Warning(
            Exception exception,
            string message = null,
            ...
            [CallerLineNumber] int lineNumber = 0)
        {
            Warn(exception, message, memberName, filePath, lineNumber);
        }
