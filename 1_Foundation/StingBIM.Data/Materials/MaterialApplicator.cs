...
namespace StingBIM.Data.Materials
{
    ...
    public class MaterialApplicator
    {
        ...
        /// <returns>Application result.</returns>
        public MaterialApplicationResult ApplyToElement(Element element, Autodesk.Revit.DB.Material material)
        {
            if (element == null)
                throw new ArgumentNullException(nameof(element));
        ...
        /// </summary>
        private Autodesk.Revit.DB.Material GetOrCreateMaterial(MaterialDefinition materialDef)
        {
            try
            {
                ...
                if (_options.CreateMaterialsIfMissing)
                {
                    using (var trans = new Transaction(_document, "Create Material"))
                    {
                        ...
                        try
                        {
                            // Material.Create returns ElementId in Revit 2019+
                            var materialId = Autodesk.Revit.DB.Material.Create(_document, materialDef.Name);
                            var newMaterial = _document.GetElement(materialId) as Autodesk.Revit.DB.Material;

                            // Set properties
                            if (!string.IsNullOrWhiteSpace(materialDef.Description))
                            {
                                // Note: Description is not directly settable on Material
                                // Store in custom parameter if available
                            }
        ...
        /// </summary>
        private void SetMaterialProperties(Autodesk.Revit.DB.Material material, MaterialDefinition materialDef)
        {
            try
            {
            ...
            {
                _logger.Warning($"Failed to set properties for material '{materialDef.Name}': {ex.Message}");
            }
        }
        ...
        /// </summary>
        private bool ApplyMaterialToElement(Element element, Autodesk.Revit.DB.Material material)
        {
            try
            {
        ...
        /// </summary>
        private bool TrySetCategoryMaterial(Element element, Autodesk.Revit.DB.Material material)
        {
            try
            {
