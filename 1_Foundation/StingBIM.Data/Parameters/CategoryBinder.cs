    ...
    public class CategoryBinder
    {
        ...
        private bool ApplyParameterBinding(ParameterDefinition paramDef, List<CategoryBinding> bindings)
        {
            try
            {
                ...
                if (definition == null)
                {
                    // Create new definition - convert StingBIMParameterType to ForgeTypeId
                    var forgeTypeId = ParameterTypeConverter.ToForgeTypeId(paramDef.RevitStingBIMParameterType);
                ...
                // Apply binding using ForgeTypeId (Revit 2022+ API)
                var groupTypeId = new ForgeTypeId("autodesk.spec.aec.groups.identity");
                if (bindingMap.Contains(definition))
                {
                    // Update existing binding
                    bindingMap.ReInsert(definition, newBinding, groupTypeId);
                }
                else
                {
                    // Insert new binding
                    bindingMap.Insert(definition, newBinding, groupTypeId);
                }
