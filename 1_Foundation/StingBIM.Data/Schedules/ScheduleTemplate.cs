...
namespace StingBIM.Data.Schedules
{
    ...
    public sealed class ScheduleTemplate : IEquatable<ScheduleTemplate>
    {
        ...
        /// <summary>
        /// Gets the schedule type (alias for Type property)
        /// </summary>
        public ScheduleType ScheduleType => Type;
        ...
        /// <summary>
        /// Gets the totals configuration
        /// </summary>
        public ScheduleTotalsConfiguration Totals { get; }

        /// <summary>
        /// Gets whether this schedule has totals configured
        /// </summary>
        ...
        {
            if (scheduleName.IndexOf("MEP", StringComparison.OrdinalIgnoreCase) >= 0 ||
                scheduleName.IndexOf("Mechanical", StringComparison.OrdinalIgnoreCase) >= 0 ||
                scheduleName.IndexOf("Electrical", StringComparison.OrdinalIgnoreCase) >= 0 ||
                scheduleName.IndexOf("Plumbing", StringComparison.OrdinalIgnoreCase) >= 0)
                return "MEP";

            if (scheduleName.IndexOf("Arch", StringComparison.OrdinalIgnoreCase) >= 0 ||
                scheduleName.IndexOf("Door", StringComparison.OrdinalIgnoreCase) >= 0 ||
                scheduleName.IndexOf("Window", StringComparison.OrdinalIgnoreCase) >= 0 ||
                scheduleName.IndexOf("Room", StringComparison.OrdinalIgnoreCase) >= 0)
                return "Architecture";

            if (scheduleName.IndexOf("Struct", StringComparison.OrdinalIgnoreCase) >= 0)
                return "Structural";
        ...
        /// </summary>
        public static ScheduleTemplate FromCsvLine(string line, char delimiter = ',')
        {
        ...
        /// <summary>
        /// Creates a ScheduleTemplate from CSV line with specified type and discipline
        /// </summary>
        /// <param name="line">CSV line</param>
        /// <param name="scheduleType">Schedule type</param>
        /// <param name="discipline">Schedule discipline</param>
        /// <param name="delimiter">CSV delimiter character</param>
        public static ScheduleTemplate FromCsvLine(string line, ScheduleType scheduleType, string discipline, char delimiter = ',')
        {
            var parts = line.Split(delimiter);
            if (parts.Length < 3)
                throw new ArgumentException($"Invalid CSV line format: {line}");
    ...
    public class ScheduleFieldDefinition
    {
        ...
        public ScheduleHorizontalAlignment Alignment { get; set; }
        ...
        /// <summary>
        /// Gets or sets whether this is a calculated field (alias for IsCalculatedValue)
        /// </summary>
        public bool IsCalculated
        {
            get => IsCalculatedValue;
            set => IsCalculatedValue = value;
        }

        /// <summary>
        /// Gets or sets the column width (alias for Width)
        /// </summary>
        public int? ColumnWidth
        {
            get => Width > 0 ? Width : (int?)null;
            set => Width = value ?? -1;
        }

        /// <summary>
        /// Gets or sets the header text (alias for Heading)
        /// </summary>
    ...
    {
        Standard,
        Regular = Standard, // Alias for Standard
        MaterialTakeoff,
        KeySchedule,
        NoteBlock,
        ViewList,
    ...
    // HorizontalAlignmentStyle is provided by Autodesk.Revit.DB
