    ...
    public class ScheduleGenerator
    {
                ...
                foreach (var fieldDef in fields.OrderBy(f => f.DisplayOrder))
                {
                    try
                    {
                        ...
                        if (schedulableField != null)
                        {
                            ...
                            scheduleField.HorizontalAlignment = fieldDef.Alignment;
            ...
            try
            {
                ...
                foreach (var filterDef in filters)
                {
                    try
                    {
                        ...
                        if (field != null)
                        {
                            // Create filter - use appropriate overload based on value type
                            var revitFilterType = ConvertFilterType(filterDef.FilterType);
                            ScheduleFilter filter;

                            // Try to parse as double for numeric filters
                            if (double.TryParse(filterDef.Value, out double numericValue))
                            {
                                filter = new ScheduleFilter(field.FieldId, revitFilterType, numericValue);
                            }
                            else
                            {
                                // Use string filter
                                filter = new ScheduleFilter(field.FieldId, revitFilterType, filterDef.Value);
                            }
        ...
        private void ApplyFormatting(ViewSchedule schedule, ScheduleFormatting formatting)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(formatting.HeaderColor))
                {
        ...
        private ScheduleField FindScheduleField(ScheduleDefinition definition, string parameterName)
        {
            try
            {
                ...
                for (int i = 0; i < fieldCount; i++)
                {
                    ...
                    // Get field name directly from ScheduleField
