...
__title__ = "Create\nPipes"
__author__ = "Author"
__doc__ = "Create Pipes types with MEP parameters from external data"

def main():
    doc = revit.doc
    output.print_md("# Create Pipes")

    cat_params = data_loader.build_category_params_dict()
    params = cat_params.get("Pipes", [])
    output.print_md("Parameters for Pipes: **{}**".format(len(params)))

    mep_mats = data_loader.load_mep_materials()
    elem_mats = [m for m in mep_mats
                 if 'PIPE' in m.get('SOURCE_SHEET', '').upper()
                 or 'PIPE' in m.get('MAT_CATEGORY', '').upper()]
    output.print_md("Materials for Pipes: **{}**".format(len(elem_mats)))
