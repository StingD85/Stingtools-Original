...
__title__ = "Create\nCable Trays"
__author__ = "Author"
__doc__ = "Create Cable Trays types with MEP parameters from external data"

def main():
    doc = revit.doc
    output.print_md("# Create Cable Trays")

    cat_params = data_loader.build_category_params_dict()
    params = cat_params.get("Cable Trays", [])
    output.print_md("Parameters for Cable Trays: **{}**".format(len(params)))

    mep_mats = data_loader.load_mep_materials()
    elem_mats = [m for m in mep_mats
                 if 'CABLE' in m.get('SOURCE_SHEET', '').upper()
                 or 'CABLE' in m.get('MAT_CATEGORY', '').upper()]
    output.print_md("Materials for Cable Trays: **{}**".format(len(elem_mats)))
