...
__title__ = "Create\nDucts"
__author__ = "Author"
__doc__ = "Create Ducts types with MEP parameters from external data"

def main():
    doc = revit.doc
    output.print_md("# Create Ducts")

    cat_params = data_loader.build_category_params_dict()
    params = cat_params.get("Ducts", [])
    output.print_md("Parameters for Ducts: **{}**".format(len(params)))

    mep_mats = data_loader.load_mep_materials()
    elem_mats = [m for m in mep_mats
                 if 'DUCT' in m.get('SOURCE_SHEET', '').upper()
                 or 'DUCT' in m.get('MAT_CATEGORY', '').upper()]
    output.print_md("Materials for Ducts: **{}**".format(len(elem_mats)))
