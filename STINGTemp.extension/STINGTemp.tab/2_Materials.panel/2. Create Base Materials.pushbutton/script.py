...
output.close_others()
__title__ = "2. Create Base\nMaterials"
__author__ = "Author"
__doc__ = "Create base Revit materials from BLE + MEP libraries"

def main():
    doc = revit.doc
    output.print_md("# Create BLE+MEP Materials")
    ble_mats = data_loader.load_ble_materials()
    mep_mats = data_loader.load_mep_materials()
    all_mats = ble_mats + mep_mats
    ...
    with revit.Transaction("Create BLE+MEP Materials"):
        for md in all_mats:
