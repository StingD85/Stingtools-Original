...
__title__ = "4. Create\nRoofs"
__author__ = "Author"
__doc__ = "Create Roofs types with parameters and materials from external data"

def main():
    doc = revit.doc
    output.print_md("# Create Roofs")

    cat_params = data_loader.build_category_params_dict()
    params = cat_params.get("Roofs", [])
    output.print_md("Parameters for Roofs: **{}**".format(len(params)))

    ble_mats = data_loader.load_ble_materials()
    elem_mats = [m for m in ble_mats
                 if 'ROOF' in m.get('SOURCE_SHEET', '').upper()
                 or 'ROOF' in m.get('MAT_CATEGORY', '').upper()]
    output.print_md("Materials for Roofs: **{}**".format(len(elem_mats)))
