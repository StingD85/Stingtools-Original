...
__title__ = "2. Create\nCeilings"
__author__ = "Author"
__doc__ = "Create Ceilings types with parameters and materials from external data"

def main():
    doc = revit.doc
    output.print_md("# Create Ceilings")

    cat_params = data_loader.build_category_params_dict()
    params = cat_params.get("Ceilings", [])
    output.print_md("Parameters for Ceilings: **{}**".format(len(params)))

    ble_mats = data_loader.load_ble_materials()
    elem_mats = [m for m in ble_mats
                 if 'CEILING' in m.get('SOURCE_SHEET', '').upper()
                 or 'CEILING' in m.get('MAT_CATEGORY', '').upper()]
    output.print_md("Materials for Ceilings: **{}**".format(len(elem_mats)))
