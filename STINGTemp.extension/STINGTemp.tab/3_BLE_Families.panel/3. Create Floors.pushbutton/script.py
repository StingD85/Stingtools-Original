...
__title__ = "3. Create\nFloors"
__author__ = "Author"
__doc__ = "Create Floors types with parameters and materials from external data"

def main():
    doc = revit.doc
    output.print_md("# Create Floors")

    cat_params = data_loader.build_category_params_dict()
    params = cat_params.get("Floors", [])
    output.print_md("Parameters for Floors: **{}**".format(len(params)))

    ble_mats = data_loader.load_ble_materials()
    elem_mats = [m for m in ble_mats
                 if 'FLOOR' in m.get('SOURCE_SHEET', '').upper()
                 or 'FLOOR' in m.get('MAT_CATEGORY', '').upper()]
    output.print_md("Materials for Floors: **{}**".format(len(elem_mats)))
